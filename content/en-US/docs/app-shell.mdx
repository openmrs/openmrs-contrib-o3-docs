...
For each module URL, the app shell appends a `<script>` tag. When the script loads, it exposes a Webpack Module
Federation container on `window` using the module name transformed into a JavaScript identifier. The app shell then calls
the container's `init` and `get` functions to load the module's `./start` export.
