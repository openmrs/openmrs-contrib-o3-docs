...
`registerBreadcrumb` and `registerBreadcrumbs` only register breadcrumb data. A page still needs to render a breadcrumb UI. Current O3 apps usually do this by rendering a shared `breadcrumbs-slot`, but that slot must have a breadcrumb extension assigned by the distribution or by the module that owns the page layout.
