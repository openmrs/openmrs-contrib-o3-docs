...
`openmrsFetch` automatically prefixes relative paths with `window.openmrsBase`, requests JSON responses by default, and stringifies plain-object request bodies before sending them.
