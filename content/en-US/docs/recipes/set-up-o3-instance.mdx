...
At this point, the backend setup script should be running. The reference application installs automatically (the interactive setup wizard is disabled), so during first boot `http://localhost/openmrs` may briefly show a progress page at `http://localhost/openmrs/initialsetup` while the database is initialized. This takes a few minutes and requires no manual steps. The O3 frontend is served at `http://localhost/openmrs/spa`; `http://localhost/openmrs` is the backend and legacy UI path.
