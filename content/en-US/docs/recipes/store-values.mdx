...
The lifecycle of this variable depends on its [scope](https://developer.mozilla.org/en-US/docs/Glossary/Scope). In general, it's preferable to store values in the narrowest scope possible, that is, prefer function scope to module scope and module scope to global scope.
...
Provided by `@openmrs/esm-framework` (specifically, `esm-state`); this gives us access to [Zustand stores](https://github.com/pmndrs/zustand). This is useful for storing values that are shared across more than one component and need to be consistent across them. Note that while the underlying implementation uses Zustand, it is preferable to use the framework-provided APIs, such as [`createGlobalStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/createGlobalStore.md) and [`useStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useStore.md), e.g.,
...
[App Context](https://github.com/openmrs/openmrs-esm-core/tree/main/packages/framework/esm-context) is a mechanism for sharing state across different parts of the application. It is global by namespace, so any application can read a registered namespace, but the namespace is owned by the component that defines it and is removed when that component unmounts.
...
- [useDefineAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useDefineAppContext.md) for defining the context.
- [useAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useAppContext.md) for accessing the context.
