...
No in-memory values survive page reload. For values that need to persist across a page reload, which means browser page refresh and not merely in-application navigation, use persistent storage.
...
- [useDefineAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/API.md#usedefineappcontext) for defining the context.
- [useAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/API.md#useappcontext) for accessing the context.
...
`useDefineAppContext()` removes the namespace when the defining component unmounts. App context namespaces should use `null` rather than `undefined` when they need to represent an empty value.
