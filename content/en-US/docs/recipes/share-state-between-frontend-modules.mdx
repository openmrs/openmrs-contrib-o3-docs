...
Use the [`useState` hook](https://react.dev/reference/react/useState) (or
[`useReducer`](https://react.dev/reference/react/useReducer))
and [Context](https://react.dev/learn/passing-data-deeply-with-context) to pass state around within your
frontend module.
...
This can come up, for example, if your frontend module has multiple extensions
...
[`createGlobalStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/createGlobalStore.md):
...
[`getGlobalStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/getGlobalStore.md)
...
Global stores are in memory by default. If a store needs to survive a browser refresh within the same tab session, pass `"sessionStorage"` as the third argument to `createGlobalStore()`. The framework currently supports in-memory stores and session-storage-backed stores.
