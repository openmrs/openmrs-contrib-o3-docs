...
The example below starts with a controlled Carbon form and then rewrites it to use React Hook Form with Zod:
...
Install the required packages in your frontend module if the package does not already declare them:
...
Configure React Hook Form to use the Zod resolver:
...
This code snippet uses `zodResolver` from `@hookform/resolvers` to validate the form data with the Zod schema defined in Step 2. The `defaultValues` prop is optional but recommended to ensure the form fields are initialized with empty strings. The generic type parameter `<z.infer<typeof changePasswordFormSchema>>` provides type inference for the form data.
