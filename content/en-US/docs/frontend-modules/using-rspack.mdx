...
- Component lifecycle exports referenced from `routes.json`
- `startupApp()` when the module needs one-time setup before its first lifecycle runs
- `importTranslation` when the module provides translation files
...
- The `./start` module exports every component lifecycle named by that module's `routes.json`.
...
- Exposes `src/index.ts` as the `./start` federation module.
