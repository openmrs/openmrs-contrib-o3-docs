...
One additional check applies to the Angular form engine repos (openmrs-ngx-formentry, openmrs-ngx-file-uploader, and esm-form-entry-app in openmrs-esm-patient-chart): before cutting a release, confirm the Angular line they ship remains in support per the [Angular version support policy](#angular-version-support-policy) below.

## Angular version support policy

This section defines when the O3 Angular form engine stack must migrate to a newer Angular major, and how to pick the target version. Use the Angular 19 to 20 migration PRs listed under [History](#history) as the template for the scope and validation of any individual migration.
