...
- Running tests through the repo script, usually `yarn test-e2e`, or with `npx playwright test` when a repo has no wrapper script.
...
When you want to write a new test case, start by creating a new spec in `e2e/specs`.
...
- Any required environment variables configured. At minimum, set `E2E_BASE_URL` to the OpenMRS base URL, such as `http://localhost:8080/openmrs`; do not include `/spa`.
...
When tests fail, Playwright can generate traces, screenshots, videos, and an HTML report depending on your configuration. In current O3 frontend repos, GitHub Actions usually uploads `playwright-report/` as an artifact. Traces and videos are written to the output directories configured in that repo's Playwright config.

## View test reports from GitHub Actions and legacy Bamboo plans

To download the report from the GitHub Actions run, or from an older Bamboo plan that still runs these tests, follow these steps:
...
Our E2E tests are executed in a dockerized environment for each pull request and push to `main` in repos that define an E2E GitHub Actions workflow.
...
2. **Dynamic lightweight frontend:** To execute automated tests, the GitHub Actions workflow builds a lightweight frontend image from a generated `spa-assemble-config.json`.
   That config packs the apps from the current repository and adds the core apps required by that repo's E2E support script, such as primary navigation, home, patient chart, or patient banner.
...
**Docker image generation:** Current GitHub Actions E2E workflows consume the pre-filled
`openmrs/openmrs-reference-application-3-backend:nightly-with-data` and
`openmrs/openmrs-reference-application-3-db:nightly-with-data` images in their docker-compose stack.
Those images are produced outside the frontend module repo, historically through a dedicated Bamboo stage within the
[REFAPP-D3X](https://ci.openmrs.org/browse/REFAPP-D3X) Bamboo job. More details are available
[here](https://talk.openmrs.org/t/using-pre-filled-docker-images-for-running-e2e-tests/40003).
...
This approach should avoid issues caused by bundler file watching and static file changes.
