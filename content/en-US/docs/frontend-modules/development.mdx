...
Current React frontend modules use ESLint 9 with the shared [`@openmrs/eslint-config`](https://github.com/openmrs/openmrs-contrib-eslint-config) flat config. A typical root `eslint.config.mjs` looks like this:
...
Keep repository-specific ignores and rule overrides in this file. Flat config does not read `.eslintignore`, and O3 repositories should not copy the shared rules into a local `.eslintrc`.

When migrating an older repository, follow the shared config's [migration steps](https://github.com/openmrs/openmrs-contrib-eslint-config#migrating-from-a-legacy-eslintrc): add `@openmrs/eslint-config` and ESLint 9, remove the plugin dependencies provided by the package, delete `.eslintrc` and `.eslintignore`, and remove legacy `--ext` flags from lint scripts. Run the repository's existing source-scoped lint command first. Use `--fix` only with the same scope and review the resulting diff.
