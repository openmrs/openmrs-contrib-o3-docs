...
<Callout type="info">
  If you are creating a new module rather than trying to understand an existing one, start with the [Creating a frontend module](/docs/recipes/create-a-frontend-module) recipe. This overview explains the moving parts; the recipe walks through the published `@openmrs/create-o3-app` generator and the template-app fallback.
</Callout>
...
Each frontend module has a root-level `package.json` file that defines its dependencies and metadata. Below is a shortened snippet from the current Template frontend module:
...
- It also exports two named lifecycle functions, `root` and `systemAdministrationFormBuilderCardLink`. These names must match the component names declared in `routes.json`. The app shell calls them when the corresponding page or extension needs to render.
