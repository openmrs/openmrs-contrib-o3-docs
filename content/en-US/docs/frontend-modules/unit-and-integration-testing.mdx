...
3. **End-to-end (e2e) tests** - browser automation that behaves like a user to click around the app and verify that it functions correctly.
...
Current React frontend modules use **Vitest** by default. Some older repos still use **Jest**, so check the module's `package.json` before copying commands or mock setup from another repo.

Why the project has been moving toward Vitest:
...
Test commands can vary by module, so check the module's `package.json` for the exact scripts. Common patterns include `yarn test`, `yarn test:watch`, and `yarn turbo test` in repos that use Turbo.

Setup files in current Vitest modules are usually named `setup-tests.ts` and are wired through `test.setupFiles` in `vitest.config.ts`. Older Jest modules may still use `setupTests.ts`. If you need to add global mocks or test utilities, use the setup file and naming convention that already exists in that repo.

## Example: Testing a frontend module

We’re going to walk through an example test suite to understand the approach taken. In this case, we’ll use the [module-management](https://github.com/openmrs/openmrs-esm-module-management-app) frontend module. This is an older Jest-based module, so the syntax uses `jest`; current Vitest modules follow the same testing shape but use `vi` for mocks. This application takes the `Manage Module` page from the OpenMRS 2.x reference application and redesigns it to work within O3. It provides an interface for users to manage backend modules. It lists all the installed modules and allows admins to control the execution of modules via the exposed `Start`, `Stop` and `Unload` actions. Users can also view detailed information about the listed modules.
...
This code mocks out the `openmrsFetch` function, stubbing out its implementation and replacing it with a promise that resolves to an object with a `results` property that's an empty array.
...
This code asks the test runner to wait until the loader gets removed from the UI. This mimics waiting for a backend request to resolve. Next, we want to start writing our assertions:
...
it("renders an error state view if there was a problem fetching module data", async () => {
  ...
  // Equivalent to returning a rejected Promise from openmrsFetch
...
The framework exports module mocks that can be used to test functionality in `@openmrs/esm-framework`. The framework ships with two mock files:
...
### Prefer shared mocks before partial mocks

The framework mock should cover the reusable framework behavior your module needs. If you find yourself mocking an individual framework function because the shared mock is missing a generally useful stub, add that stub to the appropriate mock package instead of building a separate mock structure in every test. You may have seen a pattern like this in the past:
...
Avoid this when the goal is to provide a reusable framework stub. It creates a parallel mocking structure that is harder to maintain than the centralized framework mock. Because the stubs in the framework mock are centralized, it's easier to add new stubs each time new functionality is added to the framework.
...
Targeted partial mocks are still reasonable when a specific test needs to override one existing export, such as `useSession`, `launchWorkspace2`, or a framework UI component. In Vitest, preserve the rest of the module and override only the export you need:
...
- **Choose the correct mock file**: Add stubs to the mock file that corresponds to the package containing the original functionality:
   - For `@openmrs/esm-extensions` functionality, add the stub to the `@openmrs/esm-extensions/mock` implementation.
   - For `@openmrs/esm-api` functionality, add the stub to the `@openmrs/esm-api/mock` implementation.
   - For `@openmrs/esm-state` functionality, add the stub to the `@openmrs/esm-state/mock` implementation.
   - And so on...
...
- Extend the framework mock by adding stubs for missing reusable framework behavior. Use targeted partial mocks only for test-specific overrides.
