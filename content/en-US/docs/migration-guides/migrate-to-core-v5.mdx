...
O3 provides a powerful [module loading](/docs/frontend-modules/loading-modules) system that handles loading of frontend modules in the app shell. At the time of the Core v5 migration, this system leveraged Webpack Module Federation. The current docs cover the same runtime-sharing model in the [Rspack and Module Federation guide](/docs/frontend-modules/using-rspack). However, the pre-Core v5 loader did suffer from a few historical drawbacks:
...
1. [Factor out static metadata into a `routes.json` file](/docs/migration-guides/migrate-to-core-v5#factor-out-static-metadata)
2. [Factor out dynamic metadata into a `startupApp` activator function](/docs/migration-guides/migrate-to-core-v5#factor-out-dynamic-metadata)
3. [Upgrade core dependencies](/docs/migration-guides/migrate-to-core-v5#upgrade-core-dependencies)
4. [Check out the Troubleshooting guide](/docs/migration-guides/migrate-to-core-v5#troubleshooting)
...
- `offline` - optional boolean property. Defaults to `false`. Determines whether the component renders while the browser is not connected to the internet. If false, the page will not be rendered while offline.
- `order` - older route schemas include this integer property, but the current runtime sorts pages by app name and does not use page-level `order`. Avoid relying on it for rendering order.

  <Callout emoji="ℹ️" type="info">
    The current runtime does not enforce `privilege` or `privileges` on `pages`. Gate page access inside the page itself, or expose privileged entry points as extensions with `privileges`.
  </Callout>
...
- `offline` - we'll set this to `true` explicitly so the login and logout routes can render while offline.
...
- `privileges` - `string` or `array` property that refers to the privilege(s) that a user must have in order for this extension to be rendered.
- `online` - optional boolean property. Defaults to `true`. Determines whether the component renders while the browser is connected to the internet. If false, the page will not be rendered while online.
- `offline` - optional boolean property. Defaults to `false`. Determines whether the component renders while the browser is not connected to the internet. If false, the page will not be rendered while offline.
...
- `online` - we'll use the default value `true`.
- `offline` - we'll set this to `true` for the extensions that used `sharedOnlineOfflineProps`, and leave `logout-button` as `false` because the original definition disabled it offline.
