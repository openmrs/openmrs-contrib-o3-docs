...
- `${openmrsBase}` - Base URL for the OpenMRS backend
...
The default is `true`. When enabled, a list of recently searched patients appears in the initial search results.
...
Available values are `Scheduled`, `WalkIn`, and `Virtual`; the appointments app validates these against the backend appointment kind values.
...
If the patient already has an active visit, the default check-in flow updates the appointment status to `CheckedIn` instead of opening the Start Visit workspace. A custom URL receives `${patientUuid}` and `${appointmentUuid}` as template parameters.
...
A custom checkout URL receives `${patientUuid}` and `${appointmentUuid}` as template parameters.
...
The URL template supports `${openmrsSpaBase}`, `${openmrsBase}`, and `${patientUuid}`.
...
Available column options: `patientName`, `identifier`, `location`, `serviceType`, `status`, `dateTime`, `provider`, `actions`, `visitStartTime`.
...
**Note**: Setting this to `true` requires a custom `/appointment/unScheduledAppointment` endpoint that is not currently available by default.

### Early Appointments Tab

Show the Early Appointments tab:
...
**Note**: Setting this to `true` requires a custom `/appointment/earlyAppointment` endpoint that is not currently available by default.
...
Default the patient list location filter to the user's current session location:
