...
Enable or disable the display of recently viewed patients:
...
The default is `true`. When enabled, a list of recently viewed patients appears in the initial search results.
...
**Note**: Setting this to `true` requires a custom `earlyAppointment` endpoint that is not currently available by default.
