...
- Use [`openmrsFetch`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/openmrsFetch.md) from `@openmrs/esm-framework` to fetch data from the backend. `openmrsFetch` is a wrapper around the `fetch` API that adds authentication and authorization headers and handles errors. If you pass it directly to `useSWR`, type the SWR data as `FetchResponse<T>` and read the payload from `response.data`. For most resource hooks, prefer `useOpenmrsSWR`, which supplies `openmrsFetch` and an abort signal for you.
