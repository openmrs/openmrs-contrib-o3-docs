...
- Debounce search inputs to prevent unnecessary requests to the backend. Use the [`useDebounce`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useDebounce.md) hook from `@openmrs/esm-framework` to debounce search inputs. Here's a snippet (some bits are omitted for brevity) showing how you could use the hook:
