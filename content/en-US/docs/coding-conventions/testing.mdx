...
- [Vitest](https://vitest.dev) (the default across the majority of modules) or [Jest](https://jestjs.io) (in the small number of modules still on the legacy runner), together with [React Testing Library](https://testing-library.com/react), for unit and integration testing. See the [migration guide](/docs/migration-guides/migrate-to-rspack-and-vitest) if you're moving a module from Jest to Vitest.
...
- Avoid these [common testing pitfalls](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library):
  ...
  - Don't create one-off wrapper components for `render` or `renderHook` unless the test needs providers. When providers are needed, prefer a shared helper such as `renderWithSwr` so each test gets an isolated cache.
