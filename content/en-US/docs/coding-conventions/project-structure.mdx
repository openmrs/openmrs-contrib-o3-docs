...
- Shared configuration files should generally exist at the root of the monorepo directory. These include:
  - `eslint.config.mjs` - ESLint 9 flat config. It normally composes `@openmrs/eslint-config` and contains repository-specific ignores and rule overrides.
  ...
  A typical convention is to place configuration files for the i18next parser, shared test utilities, and test setup helpers in a `tools` directory at the root of the monorepo:

  - `tools/i18next-parser.config.js` - i18next parser configuration file
  - `tools/setup-tests.ts` - Vitest test setup helper, when the repository keeps shared test setup at the root
