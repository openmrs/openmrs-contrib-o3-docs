...
1. A **workspace group** is declared in `workspaceGroups2` and initialized at runtime by calling `launchWorkspaceGroup2`. If the group has windows with icons, it renders the action menu (siderail). Only one group can be open at a time.
...
If your app owns a workspace group (i.e. the group is declared in your `routes.json`), you need to call `launchWorkspaceGroup2` to initialize it. This is typically done in your app's root component. If the group has windows with icons, initializing the group also makes its action menu available.
