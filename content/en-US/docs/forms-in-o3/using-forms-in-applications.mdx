...
- **`formSessionIntent`** (string) - The form intent used while refining the schema. It applies an intent-specific initial page and page or field behaviours such as `readonly`, `hide`, and field `defaultValue`. The form renderer extension passes `'*'` by default; when using `FormEngine` directly, pass `'*'` explicitly if you want wildcard intent behavior.
...
The `formSessionIntent` prop selects intent-specific page and question behaviours defined in the form schema. Use it when a schema has different field defaults, readonly or hidden pages or fields, or default pages for different workflows. The patient chart form renderer extension defaults this to `'*'`, but direct `FormEngine` usage leaves it unset unless you pass a value:

```tsx
<FormEngine
  patientUUID={patientUuid}
  formUUID={formUuid}
  formSessionIntent="VITALS" // Apply page and question behaviours for the VITALS intent
/>
```
...
- Read [Advanced React form capabilities](/docs/forms-in-o3/advanced-form-capabilities) before registering data sources,
  helpers, transformers, or post-submission actions
