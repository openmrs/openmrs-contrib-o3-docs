...
1. Un **groupe de workspaces** est déclaré dans `workspaceGroups2` et initialisé à l'exécution en appelant `launchWorkspaceGroup2`. Si le groupe possède des fenêtres avec icônes, il rend le menu d'actions (siderail). Un seul groupe peut être ouvert à la fois.
...
Si votre application possède un groupe de workspaces (c'est-à-dire que le groupe est défini dans votre `routes.json`), vous devez appeler `launchWorkspaceGroup2` pour l'initialiser. Cela se fait typiquement dans le composant racine de votre application. Si le groupe possède des fenêtres avec icônes, son initialisation rend aussi son menu d'actions disponible.
