...
Les schémas de formulaire O3 sont conformes à un [schéma](https://github.com/openmrs/openmrs-contrib-json-schemas/blob/main/form.schema.json) JSON standard. Les formulaires qui correspondent à ce schéma peuvent être utilisés avec le [moteur de formulaires React](/docs/key-repositories#react-form-engine) qui alimente le rendu des formulaires JSON O3. Si vous voulez qu'un formulaire HTML Form Entry s'exécute comme un formulaire React Form Engine, vous devrez le convertir en une représentation JSON compatible avec le schéma O3.

  <Callout emoji="ℹ️" type="info">
    Pour en savoir plus sur les propriétés qui composent le schéma, consultez la [référence du schéma de formulaire O3](/docs/forms-in-o3/form-schema-reference).
  </Callout>

<Callout emoji="ℹ️" type="info">
  Le dossier patient peut toujours rendre des formulaires HTML Form Entry configurés via
  `@openmrs/esm-patient-forms-app`. Utilisez ce guide de conversion lorsque vous voulez transformer ces formulaires en
  formulaires JSON O3 modifiables dans le constructeur de formulaires et rendus par le React Form Engine.
</Callout>
...
- Gestion des obs avec `answerConceptId` ou `answerConceptIds`, ainsi que des obs dont les réponses doivent être déduites du dictionnaire
- Gestion du style des obs pour les boutons radio, les cases à cocher, les listes déroulantes
- Gestion des obs de type diagnostic qui utilisent `answerClasses="Drug"`
...
1. Installez le module [HFE to O3 schema converter](https://github.com/openmrs/hfe-o3-form-schema-converter) sur le backend où vos formulaires HTML Form Entry sont disponibles.
2. Définissez la propriété globale `htmlformmigration.resources` sur un chemin absolu d'un répertoire accessible en écriture où les schémas de formulaires HTML seront exportés.
...
2. **Convertir en JSON** : Après l'exportation, cliquez sur un formulaire dans la liste pour afficher son schéma JSON converti. Vous pouvez :
   ...
   - Ouvrir la boîte de dialogue "View and copy", qui fournit un éditeur JSON pour copier ou ajuster le schéma généré
   - Copier le JSON généré dans un fichier et continuer à l'affiner dans le Constructeur de Formulaires O3

### Notes importantes

- **Structure par défaut** : Le convertisseur génère une page par défaut "Starter page" avec une seule section "Starter section" contenant toutes les questions. Vous devrez organiser manuellement les questions en pages et sections appropriées en utilisant le Constructeur de Formulaires O3.
...
- **Groupes répétables** : Certaines versions du convertisseur émettent `questionOptions.rendering: "repeat"` pour les champs `obsgroup` répétables. Les schémas O3 actuels devraient utiliser `"repeating"`. Mettez donc cette valeur à jour avant de valider ou publier le formulaire.
...
- **Alias de rendu** : Revoyez les valeurs de rendu générées par rapport aux types de champs actuels du Form Builder. En particulier, remplacez `repeat` par `repeating` pour les groupes répétables.
- **Règles de validation** : La prise en charge de la validation est limitée. Le convertisseur détecte `required="true"` sur les balises `obs`, mais le JSON de question généré actuellement n'émet pas toujours ces indicateurs required. Revoyez donc le formulaire HFE source et ajoutez manuellement les règles de validation obligatoires.
