...
| `formatDate(value, format?)` | Chaîne formatée; lève une erreur pour une date invalide. |
...
| `calcGravida(term, abortion)` | Somme entière; lève une erreur si une entrée n'est ni un entier ni une chaîne représentant un entier. |
...
La validation du constructeur est distincte. Lorsqu'elle est activée, elle vérifie les concepts, leurs réponses, les types de métadonnées et les associations configurées entre types de données de concepts et rendus. Elle n'exécute pas tous les chemins d'exécution et ne prouve pas la prise en charge par un autre moteur.
