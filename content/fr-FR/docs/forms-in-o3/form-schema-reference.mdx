...
La validation par rapport au schéma JSON ne prouve pas que chaque moteur implémente le même comportement. Utilisez cette page pour le flux React par défaut et consultez les [implémentations du moteur de formulaires](/docs/forms-in-o3/form-engine-implementations) avant d'utiliser un autre renderer.
...
| `formOptions.usePreviousValueDisabled` | Désactive le flux de valeur précédente. |
| `meta.programs` | Fournit des métadonnées React pour les programmes et peut être transformé en action post-soumission. |

Le schéma normatif déclare aussi `availableIntents`, tandis que le moteur React applique les `behaviours` correspondants avec ses utilitaires de chargement. Ces capacités propres à React sont décrites dans les [capacités avancées](/docs/forms-in-o3/advanced-form-capabilities).
