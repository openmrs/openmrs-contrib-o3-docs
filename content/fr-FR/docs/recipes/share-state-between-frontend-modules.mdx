...
Les global stores sont stockés en mémoire par défaut. Si un store doit survivre à un rafraîchissement du navigateur dans la même session d'onglet, passez `"sessionStorage"` comme troisième argument à `createGlobalStore()`.
