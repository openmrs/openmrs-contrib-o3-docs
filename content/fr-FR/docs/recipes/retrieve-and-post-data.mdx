...
import { type FetchResponse, fhirBaseUrl, openmrsFetch, fhir } from "@openmrs/esm-framework";

const { data, error, isLoading, mutate } = useSWR<FetchResponse<fhir.Patient>, Error>(
...
export function useVisits() {
  ...
  const { data, error, isLoading, mutate } = useSWR<FetchResponse<VisitData>, Error>(url, openmrsFetch);
...
`openmrsFetch` préfixe automatiquement les chemins relatifs avec `window.openmrsBase`, demande des réponses JSON par défaut et sérialise les corps de requête qui sont des objets simples avant de les envoyer.
