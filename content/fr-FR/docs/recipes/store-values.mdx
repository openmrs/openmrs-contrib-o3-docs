...
Fourni par `@openmrs/esm-framework` (spécifiquement, `esm-state`); cela nous donne accès aux [Zustand stores](https://github.com/pmndrs/zustand). Ceci est utile pour stocker des valeurs qui sont partagées entre plusieurs composants et qui doivent être cohérentes entre elles. Notez que bien que l'implémentation sous-jacente utilise Zustand, il est préférable d'utiliser les API fournies par le framework, comme [`createGlobalStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/createGlobalStore.md) et [`useStore`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useStore.md), par exemple,
...
[App Context](https://github.com/openmrs/openmrs-esm-core/tree/main/packages/framework/esm-context) est un mécanisme de partage d'état entre différentes parties de l'application. Il est global par namespace, donc toute application peut lire un namespace enregistré, mais ce namespace appartient au composant qui le définit et il est supprimé lorsque ce composant est démonté.
...
- [useDefineAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useDefineAppContext.md) pour définir le contexte.
- [useAppContext](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useAppContext.md) pour accéder au contexte.
