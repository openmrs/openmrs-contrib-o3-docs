...
Configurez les informations qui apparaissent sur les cartes de patients des services. Chaque élément configuré doit avoir un `id` unique qui correspond à la prop `id` transmise par le composant React du service utilisé.
...
**Propriétés** :
- `id` - Identifiant unique pour cet élément (requis). Il doit correspondre à l'`id` du composant de carte du service; les cartes de service maternité intégrées utilisent `pregnancy-complications`.
...
Les tags d'observation colorés ne prennent en compte que les observations de la visite actuelle du patient.
Les cartes de service intégrées traduisent `summaryLabel` avec l'espace de noms de traduction de l'application de service.
...
## Concepts supplémentaires de notes d'hospitalisation

Configurez des concepts de notes supplémentaires à afficher dans l'espace de travail des notes du patient du service. Ils sont affichés avec le `consultFreeTextCommentsConcept` fourni par EMR API.
...
## Restrictions du bouton de sortie par lieu de session

Le bouton de sortie du menu d'actions peut être limité à des lieux de session précis :
...
Quand `allowedSessionLocationUuids` est vide, le bouton de sortie est disponible dans tous les lieux de session. Quand il contient des UUIDs, le bouton ne s'affiche que pour les utilisateurs dont le lieu de session actuel figure dans la liste.
