...
L'app des files d'attente de service fournit l'extension `visit-form-queue-fields` au formulaire de début de visite du dossier patient. Ces champs sont rendus lorsque la configuration du dossier patient active les champs de file de service, ou lorsque le formulaire de début de visite est ouvert depuis le flux d'ajout de patient aux files de service :
...
Les onglets de types de visite recommandés sont également contrôlés par le formulaire de début de visite du dossier patient. Configurez `showRecommendedVisitTypeTab` et `visitTypeResourceUrl` sur `@openmrs/esm-patient-chart-app`, pas sur `@openmrs/esm-service-queues-app`.
...
- `extension` - Colonne personnalisée rendue par un emplacement d'extension
...
- `config` - Configuration spécifique à la colonne :
  ...
  - Pour `extension` : toute colonne d'extension rend l'emplacement d'extension `queue-table-<column-id>-slot` et transmet `queueEntry` dans l'état de l'emplacement. Par exemple, une colonne avec l'ID `eligibility` rend `queue-table-eligibility-slot`.
...
Ces types d'identifiants sont utilisés lorsque les détails d'une entrée de file doivent afficher des identifiants préférés, par exemple dans la modale d'appel du patient.
