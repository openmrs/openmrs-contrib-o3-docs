...
**Note** : Les schémas de configuration sont optionnels. Si votre module n'a pas besoin d'options de configuration (par exemple, s'il s'agit d'un module bibliothèque comme le [React Form Engine](https://github.com/openmrs/openmrs-esm-form-engine-lib) qui reçoit toute sa configuration via des props ou des paramètres de fonction), vous n'avez pas besoin de définir un schéma. Le système de configuration fonctionnera toujours; les modules sans schéma utilisent le schéma implicite pour `Display conditions` et `Translation overrides`.
...
```ts
// Ce n'est pas correct. `salutation` est une valeur brute, pas un objet de schéma.
defineConfigSchema("@openmrs/esm-hologram-doctor", {
  hologram: {
...
```ts
// Ne faites pas ça. `_default` est un mot-clé de schéma, pas un nom d'élément de configuration.
defineConfigSchema("@openmrs/esm-hologram-doctor", {
  hologram: {
    salutation: {
      _default: {
...
- `Type.Object` - Valeurs d'objet libres. Lorsqu'il est utilisé avec `_elements`, O3 valide et applique les valeurs par défaut pour chaque valeur de l'objet.
...
Le premier argument est une fonction qui prend la valeur de configuration comme seul argument. Si la fonction renvoie `true`, la validation réussit. Si elle renvoie `false`, la validation échoue et le deuxième argument de `validator` est utilisé comme message d'erreur. Vous pouvez aussi fournir directement une fonction de validation personnalisée dans `_validators`; les validateurs directs doivent renvoyer `undefined` en cas de succès ou une chaîne d'erreur en cas d'échec.
...
Pour des raisons de commodité, certains validateurs courants sont fournis d'office, notamment [`inRange`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/inRange.md), [`oneOf`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/oneOf.md) et [`isUrlWithTemplateParameters`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/isUrlWithTemplateParameters.md).

#### Tableaux

Vous pouvez accepter et valider des tableaux, et des tableaux contenant des objets, dans votre schéma de configuration. Ceci est configuré avec le paramètre `_elements`, utilisé avec `_type: Type.Array`. Par exemple, un schéma qui accepterait un tableau de chaînes de caractères d'une longueur maximale de 30 caractères ressemblerait à ceci:
...
Ce schéma valide les clés et les types de valeurs des objets du tableau `robots`. Les clés inconnues sont consignées comme erreurs de configuration, et les valeurs par défaut sont appliquées lorsqu'une entrée de `_elements` fournit une `_default`.

Les objets dans les tableaux n'ont pas besoin de fournir des valeurs par défaut pour chaque clé. Si un champ comme `name` est indispensable au bon fonctionnement de votre module, ajoutez un validateur qui vérifie chaque objet et renvoie un message d'erreur utile lorsque le champ est absent.
...
Les objets imbriqués sont fusionnés en profondeur, mais les tableaux sont remplacés en bloc. Si une source de configuration plus tardive fournit un tableau pour une clé, il remplace le tableau précédent au lieu d'être fusionné par index.
...
export default function DoctorGreeting() {
  ...
  const greeting = "Bonjour, mon nom est Dr. " + config.virtualProvider.name.given.join(" ");
...
Le nom de l'extension doit correspondre à la propriété `name` de l'extension telle que définie dans `routes.json`. Lorsqu'une extension a son propre schéma de configuration, les valeurs de base de ce schéma sont fournies sous le nom de l'extension, pas sous le nom du module. `useConfig()` dans cette extension retourne toujours la configuration normale du module plus la configuration de l'extension. Les valeurs spécifiques à l'extension ont la priorité lorsque la même clé de premier niveau existe aux deux endroits.
...
1. **Configuration de base de l'extension** - Valeurs fournies sous le nom de l'extension lorsque l'extension a son propre schéma, ou sous le nom du module de l'extension lorsqu'elle utilise le schéma du module
...
La clé dans `configure` doit correspondre à l'ID d'extension assigné au slot. Si vous attachez plusieurs fois la même extension avec des IDs d'instance comme `my-extension#left` et `my-extension#right`, configurez ces IDs complets séparément.

Par exemple, ceci fournit la configuration de base de l'extension :
...
La configuration de l'extension peut ensuite être remplacée par slot en utilisant l'objet `configure` dans la configuration du slot :
...
Tous les éléments de configuration doivent avoir une valeur par défaut (à l'exception des éléments dans les tableaux ou les objets libres gouvernés par `_elements`).
...
Quelques validateurs courants sont fournis, notamment [`inRange`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/inRange.md), [`oneOf`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/oneOf.md) et [`isUrlWithTemplateParameters`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/isUrlWithTemplateParameters.md).

Les validateurs personnalisés doivent être créés avec la fonction [`validator`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/validator.md).

Peut être utilisé n'importe où dans la structure du schéma.

`_elements`

Valable uniquement avec `_type: Type.Array` ou `_type: Type.Object`. Un `_default` doit également être fourni à ce niveau. Pour les tableaux, la valeur doit être un schéma pour chaque élément du tableau. Pour les objets libres, la valeur doit être un schéma pour chaque valeur de l'objet.

### Documentation de l'API

Voir la documentation d'API générée pour [`defineConfigSchema`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/defineConfigSchema.md), [`defineExtensionConfigSchema`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/defineExtensionConfigSchema.md), [`getConfig`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/getConfig.md), [`useConfig`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/useConfig.md) et [`validator`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/validator.md).
