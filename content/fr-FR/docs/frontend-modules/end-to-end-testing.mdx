...
- D'exécuter les tests via le script du dépôt, généralement `yarn test-e2e`, ou avec `npx playwright test` quand le dépôt n'a pas de script wrapper.
...
Lorsque vous voulez écrire un nouveau scénario de test, commencez par créer une nouvelle spécification dans `e2e/specs`.
...
- Les variables d'environnement requises configurées. Au minimum, définissez `E2E_BASE_URL` sur l'URL de base OpenMRS, par exemple `http://localhost:8080/openmrs`; n'incluez pas `/spa`.
...
Lorsque les tests échouent, Playwright peut générer des traces, des captures d'écran, des vidéos et un rapport HTML selon la configuration. Dans les dépôts frontend O3 actuels, GitHub Actions téléverse généralement `playwright-report/` comme artifact. Les traces et vidéos sont écrites dans les répertoires de sortie configurés dans la configuration Playwright du dépôt.

## Afficher les rapports de test depuis GitHub Actions et les anciens plans Bamboo

Pour télécharger le rapport depuis une exécution GitHub Actions, ou depuis un ancien plan Bamboo qui exécute encore ces tests, procédez comme suit:
...
Nos tests E2E sont exécutés dans un environnement dockerisé pour chaque pull request et chaque push vers `main` dans les dépôts qui définissent un workflow GitHub Actions E2E.
...
2. **Frontend léger et dynamique:** Pour exécuter les tests automatisés, le workflow GitHub Actions construit une image frontend légère à partir d'un `spa-assemble-config.json` généré.
   Cette configuration empaquette les apps du dépôt courant et ajoute les apps de base nécessaires au script de support E2E de ce dépôt, comme la navigation principale, la page d'accueil, le dossier patient ou le bandeau patient.
...
**Génération d'images Docker:** Les workflows GitHub Actions E2E actuels consomment les images pré-remplies
`openmrs/openmrs-reference-application-3-backend:nightly-with-data` et
`openmrs/openmrs-reference-application-3-db:nightly-with-data` dans leur stack docker-compose.
Ces images sont produites en dehors du dépôt de module frontend, historiquement via une étape Bamboo dédiée dans le job
[REFAPP-D3X](https://ci.openmrs.org/browse/REFAPP-D3X). Plus de détails sont disponibles
[ici](https://talk.openmrs.org/t/using-pre-filled-docker-images-for-running-e2e-tests/40003).
...
Cette approche devrait éviter les problèmes causés par le file watching du bundler et les modifications de fichiers statiques.
