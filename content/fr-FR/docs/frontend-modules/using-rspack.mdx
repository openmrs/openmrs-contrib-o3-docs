...
<Callout type="info">
  Rspack accepte le format de configuration de Webpack. Tout ce qui est décrit sur cette page s'applique donc aussi aux applications qui utilisent encore Webpack. Les quelques applications qui n'ont pas encore migré récupèrent les mêmes valeurs par défaut via `openmrs/default-webpack-config`. Voir [Vous utilisez encore Webpack ?](#vous-utilisez-encore-webpack) plus bas.
</Callout>

## Mise en place

La configuration minimale, et celle que la plupart des applications utilisent, tient en une ligne:

```js filename="rspack.config.js"
module.exports = require("openmrs/default-rspack-config");
```

Cette ligne délègue tout à la configuration par défaut décrite ci-dessous.
...
Chaque module fédéré qui expose des extensions ou des pages pour l'app shell doit exposer un module nommé `"./start"`. Ce module doit exporter:

- `importTranslation`
- `startupApp()`
- Les exports de composants référencés depuis `routes.json`

Dans la configuration Rspack par défaut, cela est câblé en exposant le fichier `index.ts` du module sous le nom `"./start"`. Si vous chargez vous-même du code avec `importDynamic` depuis `@openmrs/esm-framework`, vous pouvez viser un autre module exposé. Mais tout ce qui est exposé ailleurs doit être enregistré manuellement avant que l'app shell puisse l'utiliser.
...
- Expose `src/index.ts` comme point d'entrée de fédération.
...
- La configuration par défaut est fournie sous le nom [`openmrs/default-webpack-config`](https://github.com/openmrs/openmrs-esm-core/tree/main/packages/tooling/webpack-config). Utilisez-la dans `webpack.config.js` exactement comme les applications Rspack utilisent `openmrs/default-rspack-config`.
