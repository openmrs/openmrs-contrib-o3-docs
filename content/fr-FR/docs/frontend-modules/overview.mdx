...
<Callout type="info">
  Si vous créez un nouveau module plutôt que de comprendre un module existant, commencez par la recette [Création d'un module frontend](/docs/recipes/create-a-frontend-module). Cette vue d'ensemble explique les pièces en jeu; la recette décrit le générateur publié `@openmrs/create-o3-app` et la solution de secours avec l'application template.
</Callout>
...
Chaque module frontend a un fichier `package.json` de niveau racine qui définit ses dépendances et ses métadonnées. Voici un extrait raccourci de l'application Template actuelle:
...
- Il exporte également deux fonctions de lifecycle nommées, `root` et `systemAdministrationFormBuilderCardLink`. Ces noms doivent correspondre aux composants déclarés dans `routes.json`. L'app shell les appelle quand la page ou l'extension correspondante doit être rendue.
