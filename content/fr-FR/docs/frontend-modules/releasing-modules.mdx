...
Créer une release O3 implique deux étapes:

1. Incrémenter les versions locales de vos packages et pousser ces changements vers GitHub. Cette étape déclenche le job CI `pre-release`.
2. Créer une release depuis l'interface GitHub Releases. Cette étape déclenche le job CI `release`, qui publie le ou les packages sur le registre npm.
...
À ce stade, vous pouvez passer à votre IDE pour lancer le processus de release. Nous utilisons généralement `yarn` pour versionner les modules frontend.
...
Créez une nouvelle branche nommée `chore/release-vX.X.X`, où `X.X.X` est le numéro de version que vous publiez. Committez ces changements avec le titre `(chore) Release vX.X.X`. Voir cet exemple de [commit](https://github.com/openmrs/openmrs-esm-patient-chart/pull/1279) qui incrémente `Patient Chart` vers `v5.0.0`.

Une fois le commit de release fusionné, le job `pre-release` du workflow CI se déclenche et publie une version pre-release taguée `next` sur npm. C'est utile pour les tests anticipés avant de publier `latest`. Sous le capot, ce job exécute le script `ci:publish-next`.
...
<Callout type="info">
  Les notes de release doivent suivre le format du [changelog O3](/docs/changelog). Ce format comprend un titre, une description et une liste de changements. Les principales catégories sont `Features`, `Bug Fixes`, `Breaking Changes`, `Refactors`, `Docs` et `Tests`. Chaque catégorie doit contenir une liste de changements. Il faudra peut-être un peu de travail pour que le changelog corresponde à ce format. Cela en vaut la peine, car cela facilite la revue des changements par le Release Manager et la préparation des notes de release pour la communauté.
</Callout>
...
Cette commande publie un nouveau package tagué `latest`. C'est la version que les consommateurs obtiendront par défaut quand ils installeront votre module frontend.

De façon générale, quand une PR est fusionnée, un job `pre-release` dans notre workflow GitHub Actions principal est déclenché. Ce job publie une version du module frontend correspondant sur son registre npm avec le tag `next`.
...
- `.github/workflows/ci.yml` pour voir quels tags sont publiés sur `push` et sur `release`.
