...
Les modules frontend React actuels utilisent ESLint 9 avec la configuration à plat partagée [`@openmrs/eslint-config`](https://github.com/openmrs/openmrs-contrib-eslint-config). Un fichier racine `eslint.config.mjs` typique ressemble à ceci:
...
Conservez les exclusions et les dérogations de règles propres au dépôt dans ce fichier. La configuration à plat ne lit pas `.eslintignore`, et les dépôts O3 ne doivent pas recopier les règles partagées dans un fichier `.eslintrc` local.

Lors de la migration d'un ancien dépôt, suivez les [étapes de migration](https://github.com/openmrs/openmrs-contrib-eslint-config#migrating-from-a-legacy-eslintrc) de la configuration partagée: ajoutez `@openmrs/eslint-config` et ESLint 9, supprimez les dépendances de plugins fournies par le package, supprimez `.eslintrc` et `.eslintignore`, puis retirez les anciens indicateurs `--ext` des scripts de lint. Exécutez d'abord la commande de lint existante du dépôt, limitée aux fichiers source. N'utilisez `--fix` qu'avec le même périmètre et vérifiez le diff obtenu.
