...
Cette commande démarre un serveur de développement Webpack avec des copies locales des modules Medications et Orders. Les changements locaux devraient se propager au serveur de développement dans votre navigateur.
...
Cela devrait démarrer un serveur de développement sur le port 7001, le prochain port disponible après 7000. Pour voir le JavaScript bundlé de l'app, ouvrez http://localhost:7001/openmrs-esm-implementer-tools-app.js dans votre navigateur. Cette URL est le point d'entrée que l'app shell utilise pour charger l'app implementer tools. Il est important qu'elle pointe vers le bon emplacement du JavaScript bundlé. Vous voudrez souvent préciser un port différent pour vous assurer que l'URL pointe vers le bon serveur plutôt que vers le port par défaut ou celui assigné par l'outillage.

<Callout type="info">
**Numéros de port**

Par défaut, le serveur de développement Webpack s'exécute sur le port 8080. Si vous lancez plusieurs serveurs de développement, le prochain port disponible est assigné au serveur suivant. Par exemple, si un serveur tourne sur 8080 et que vous en lancez un autre, il tournera sur 8081. Le suivant utilisera 8082, et ainsi de suite. C'est important pour les surcharges d'import map. Si vous lancez un module frontend avec `yarn start` sur le port 8080, l'URL du JavaScript bundlé sera http://localhost:8081/openmrs-esm-form-builder-app.js. Le nom de cette URL correspond au nom utilisé dans l'entrée [browser](https://github.com/openmrs/openmrs-esm-form-builder/blob/main/package.json#L6) du `package.json` du module frontend. Si vous exécutez un module sur un autre port, ajustez l'URL en conséquence.
...
La plupart du temps, les étapes précédentes suffisent à lancer un serveur de développement local. L'une des exceptions notables est l'exécution directe de l'`app shell`. Vous devrez lancer le shell directement si vous voulez:
...
Oui, absolument. Nous avons une suite de tests pour les modules frontend du monorepo. Selon le dépôt, nous utilisons [Jest](https://jestjs.io/) ou [Vitest](https://vitest.dev/), ainsi que [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/) pour les composants React. Consultez le `package.json` du module pour voir quel runner est configuré. Voir le guide de [tests unitaires et d'intégration](/docs/frontend-modules/unit-and-integration-testing) et le guide de [tests de bout en bout](/docs/frontend-modules/end-to-end-testing) pour plus d'informations. Envisagez aussi d'ajouter des tests E2E pour vos changements. Lire le guide de tests et les tests existants dans la base de code vous donnera une bonne idée de la façon d'en écrire.
...
- `lint` - lance un contrôle lint avec Prettier.
...
Maintenir les dépendances à jour est important pour bénéficier des dernières fonctionnalités, améliorations de performance et correctifs de sécurité. Yarn 3 fournit un outil interactif pratique pour cela:
...
Si vous exécutez ou construisez le package `form-entry`, un wrapper Angular autour d'AMPATH Form Engine, avec Node v18, vous pouvez voir l'erreur suivante:
...
Pour les corriger, mettez à jour les déclarations de types ambiantes de Carbon. Ajoutez ces entrées dans `declarations.d.ts`:
