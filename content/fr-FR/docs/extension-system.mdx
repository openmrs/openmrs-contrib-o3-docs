...
La configuration d'un slot se place sous le module qui possède ce slot. Dans cet exemple, `@openmrs/esm-patient-chart` est le module qui rend `patient-header-slot`.
...
Les slots peuvent accéder à la méta via l'API du système d'extension, par exemple en utilisant [useExtensionSlotMeta](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/API.md#useextensionslotmeta). `meta` vient de la définition de l'extension; utilisez les réglages `configure` du slot quand les administrateurs doivent remplacer une configuration spécifique à l'extension.
...
- Vérifiez `privileges`, `displayExpression`, `featureFlag` et les champs `online`/`offline` de la définition de l'extension.
