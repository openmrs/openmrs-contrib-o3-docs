...
- Évitez ces [pièges de test courants](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library) :
  ...
  - Ne créez pas de composants wrapper ponctuels pour `render` ou `renderHook` sauf si le test a besoin de providers. Lorsque des providers sont nécessaires, préférez un helper partagé comme `renderWithSwr` afin que chaque test obtienne un cache isolé.
