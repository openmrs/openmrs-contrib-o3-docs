...
- Fournissez des dépendances appropriées pour les hooks qui acceptent des tableaux de dépendances (`useEffect`, `useMemo`, `useCallback`, `useLayoutEffect`, `useInsertionEffect` et `useImperativeHandle`). Cela aide à garantir que vos hooks restent synchronisés avec les dernières props et l'état de votre composant. La règle générale est de :
