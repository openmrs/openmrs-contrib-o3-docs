...
- Utilisez [openmrsFetch](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/API.md#openmrsfetch) depuis `@openmrs/esm-framework` pour récupérer les données du backend. `openmrsFetch` est un wrapper autour de l'API `fetch` qui ajoute des en-têtes d'authentification et d'autorisation et gère les erreurs. Si vous le passez directement à `useSWR`, typez les données SWR comme `FetchResponse<T>` et lisez la charge utile depuis `response.data`. Pour la plupart des hooks de ressource, préférez `useOpenmrsSWR`, qui fournit `openmrsFetch` et un signal d'abandon pour vous.
