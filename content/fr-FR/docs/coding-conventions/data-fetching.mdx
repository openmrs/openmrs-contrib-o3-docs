...
- Utilisez [`openmrsFetch`](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-framework/docs/functions/openmrsFetch.md) depuis `@openmrs/esm-framework` pour récupérer les données du backend. `openmrsFetch` est un wrapper autour de l'API `fetch` qui résout les URLs relatives à OpenMRS, définit des en-têtes adaptés au JSON, gère les en-têtes UI de l'API REST OpenMRS, analyse les corps de réponse et lève une erreur pour les réponses non-2xx. Si vous le passez directement à `useSWR`, typez les données SWR comme `FetchResponse<T>` et lisez la charge utile depuis `response.data`. Pour la plupart des hooks de ressource, préférez `useOpenmrsSWR`, qui fournit `openmrsFetch` et un signal d'abandon pour vous.
